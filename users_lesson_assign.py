...
class Ui_Lesson_Assign(QDialog):
    ...
    def get_init_data(self):
        ...
        tool_button_arr = [
            ...
            ["시간표 미리보기", "img/lesson_schedule.png", 40, 40, self.showTimetable]
        ]
    ...
    def resetInfo(self):
        ...
        if configData['message'] == 'Y':
            ...
            if self.radioButton_2.isChecked():
                ...
                lesson_assign_df_dae_origin.to_excel('data/lesson_assign_dae_tableview.xlsx',
                                  index=False)  # dataframe excel 저장
                self.df_dae_load
            self.tableData()
        else:
    ...
    def randomAssign(self):
        ...
        if self.radioButton_2.isChecked():
            ...
            for i in range(len(professor_sorted_assign_list)):
                ...
                # 랜덤
                for j in range(len(grad_data_sort_list)):
                    # 새로오신 교수님은 데이터셋에 없음... 그래서 아쉽지만 continue....
                    if len(grad_data_sort_list[j]) == 0:
                        continue
                    ...
                    #     if class_arr[4]
                    diff_arr = []
                    diff_arr = [class_arr[0], class_arr[4], '']
                    for k in class_arr[3].split(','):
                        if "월" in class_arr[2]:
                            for l in range(len(lesson_assign_arr[int(k)][0])):
                                for m in range(len(diff_arr)):
    ...
    def saveInfo(self):
        ...
        if configData['message'] == 'Y':
            if radioBtn2.isChecked():  # 학부 라디오 버튼이 체크 되어있을 때, 학부 파일 lesson_assign_under_list 를 데이터프레임으로 저장
                ...
                lesson_assign_df_dae.to_excel('data/lesson_assign_dae_tableview.xlsx',
                                                index=False)  # dataframe excel 저장
                global_funtion().message_box_1(QMessageBox.Information, "정보", "저장되었습니다", "확인")
    ...
    def lessonSubmit(self):
        ...
        for i in range(len(lesson_assign_list)):
            if lesson_assign_list[i][9] == year_str and lesson_assign_list[i][10] == semester_str:
                global_funtion().message_box_1(QMessageBox.Information, "경고", "이미 해당 학기에 확정된 강의 시간표가 있습니다.", "확인")
                return
        lesson_assign_under_df_sub = pd.read_excel('data/lesson_assign_under.xlsx')
        lesson_assign_df_dae_sub = pd.read_excel('data/lesson_assign_dae.xlsx')
        ...
        lesson_dataset_list = []
        lesson_submit_total_list = lesson_assign_under_list_sub + lesson_assign_list_dae_sub
        for i in range(len(lesson_submit_total_list)):
            for j in range(len(lesson_submit_total_list[i])):
                if lesson_submit_total_list[i][j] == '':
            ...
            submit_list = [lesson_submit_total_list[i][0], professor_class_dictionary[lesson_submit_total_list[i][0]],
                           ...
                           lesson_submit_total_list[i][1], str(lesson_submit_total_list[i][2]), classtime_str,lesson_submit_total_list[i][6], '']
